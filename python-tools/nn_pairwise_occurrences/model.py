...
def normalize_data(data: dict, feature_indices: Dict[str, int], missing_features: list[tuple[str, str]]) -> tuple[list[float], float]:
    ...
    # Merge 'dependentPairs' and 'independentPairs' by iterating both.
    dep = data.get("dependentPairs", {}) or {}
    indep = data.get("independentPairs", {}) or {}

    pairs_map: Dict[str, int] = {}
    for k, v in dep.items():
        pairs_map[k] = pairs_map.get(k, 0) + int(v)
    for k, v in indep.items():
        pairs_map[k] = pairs_map.get(k, 0) + int(v)

    # total_pairs used for normalization (avoid divide-by-zero)
    total_pairs = sum(int(v) for v in pairs_map.values()) or 1

    for pair, count in pairs_map.items():
