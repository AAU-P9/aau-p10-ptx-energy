...
predictions = model.predict(kernel_xs)
predictions_original = np.expm1(predictions)
actual_original = np.expm1(ys)

print("\nPredictions:")
print(f"{'Filename':<60} {'Log-scaled Pred':<20} {'Original Pred':<20} {'Log-scaled Actual':<20} {'Original Actual':<20}")
print("-" * 140)
for i, filename in enumerate(kernel_names):
    print(f"{filename:<60} {predictions[i][0]:<20.6f} {predictions_original[i][0]:<20.6f} {ys[i][0]:<20.6f} {actual_original[i][0]:<20.6f}")
