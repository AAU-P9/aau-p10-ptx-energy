...
int main() {
    ...
    // shared mem per block = sizeof(double) * tpb_j * (3*25 + 2*5)
    // tpb_j limited by shared mem; use conservative 4
    size_t tpb_j = 4;
    size_t wx = round_work(JMAX-2, tpb_j);
    size_t wy = round_work(PROBLEM_SIZE*5, 5);
    dim3 block(wx/tpb_j, wy/5, 1);
    dim3 thread(tpb_j, 5, 1);
    size_t smem = sizeof(double) * tpb_j * (3*5*5 + 2*5);

    printf("[LOG] bt_x_solve_3: PROBLEM_SIZE=%d, ITERATIONS=%d\n", PROBLEM_SIZE, ITERATIONS);
