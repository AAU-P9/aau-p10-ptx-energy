...
#define rtmp(m,i,j,k) rhstmp[(i)+nx*((k)+nz*((j)+ny*(m)))]
	...
	META_LOOP(j_sweep_3, 1, NY, false);
	for(j=0; j<ny-2; j++){
		#pragma unroll
...
int main() {
    ...
    printf("[LOG] sp_y_solve_gpu_kernel: NX=%d NY=%d NZ=%d ITERATIONS=%d\n", NX, NY, NZ, ITERATIONS);
    dim3 grid(1, NZ);
    sp_kernel<<<grid, NX>>>(rho_i, vs, speed, rhs, lhs, rhs_buf, NX, NY, NZ);
    ...
    cudaFree(rho_i); cudaFree(vs); cudaFree(speed); cudaFree(rhs); cudaFree(lhs); cudaFree(rhs_buf);
    return 0;
}
