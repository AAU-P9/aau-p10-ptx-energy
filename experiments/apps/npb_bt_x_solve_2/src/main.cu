...
__device__ void x_solve_gpu_device_fjac(double fjac[5][5], 
		double t_u[5], 
		double rho_i,
		double qs, 
		double square){
...
__device__ void x_solve_gpu_device_njac(double njac[5][5], 
		double t_u[5],
		double rho_i){
...
int main() {
    ...
    size_t tpb = 32;
    size_t wx = round_work(IMAX-2, 1);
    size_t wy = round_work(JMAX-2, tpb);
    size_t wz = round_work(PROBLEM_SIZE, 1);
    dim3 block(wx, wy/tpb, wz);
    dim3 thread(1, tpb, 1);
