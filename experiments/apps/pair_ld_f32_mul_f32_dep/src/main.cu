...
// Dependent pair: ld.global.f32 -> mul.f32
// The loaded value is immediately multiplied (RAW dependency).
// Contrasts with ld->add dep to isolate whether multiply latency differs from add.
...
__global__ void ptx_kernel(const float * __restrict__ data, float *out, int len)
{
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        ...
        asm volatile("mul.f32 %0, %0, %1;" : "+f"(acc) : "f"(val));
    }
...
int main()
{
    ...
    for (int i = 0; i < len; i++)
        h_data[i] = 1.0f + (float)(i % 100) * 0.00001f;
    ...
    printf("[LOG] pair_ld_f32_mul_f32_dep: %d iterations, dep pair ld->mul\n", ITERATIONS);
