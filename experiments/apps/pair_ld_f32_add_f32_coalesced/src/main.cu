...
// Dependent pair: ld.global.f32 -> add.f32  (coalesced access)
// Consecutive threads access consecutive addresses — warp coalesces into a single
// 128-byte transaction per load. Best-case global memory bandwidth utilisation.
// Pair with pair_ld_f32_add_f32_strided to isolate the energy cost of coalescing.
...
__global__ void ptx_kernel(const float * __restrict__ data, float *out, int len)
{
    ...
    // Coalesced: thread N always reads element (base + N) for some base.
    // Consecutive threads → consecutive addresses → single transaction per warp.
...
int main()
{
    ...
    printf("[LOG] pair_ld_f32_add_f32_coalesced: %d iterations, coalesced ld->add\n", ITERATIONS);
