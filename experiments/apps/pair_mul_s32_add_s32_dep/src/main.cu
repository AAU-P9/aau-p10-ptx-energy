...
// Dependent pair: mul.lo.s32 -> add.s32
// The multiply result feeds directly into the add (RAW dependency).
// Common pattern in index calculations and loop induction.
...
int main()
{
    ...
    printf("[LOG] pair_mul_s32_add_s32_dep: %d iterations, dep pair\n", ITERATIONS);
