...
int main() {
    ...
    cudaMalloc(&forcing, BUF_5D); cudaMemset(forcing, 0, BUF_5D);

    size_t tpb = 32;
    size_t wx = round_work(IMAX*5, tpb);
    size_t wy = round_work(JMAX, 1);
    size_t wz = round_work(KMAX, 1);
    dim3 block(wx/tpb, wy, wz);
    dim3 thread(tpb, 1, 1);

    printf("[LOG] bt_compute_rhs_2: PROBLEM_SIZE=%d, ITERATIONS=%d\n", PROBLEM_SIZE, ITERATIONS);
    for (int it = 0; it < ITERATIONS; it++) {
        bt_kernel<<<block, thread>>>(rhs, forcing);
    }
