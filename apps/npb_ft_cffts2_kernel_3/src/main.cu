...
__global__ void ft_kernel(dcomplex x_out[], 
		dcomplex y0[]){
...
int main() {
    ...
    dcomplex *x_out; cudaMalloc(&x_out, BUF_NTOTAL_COMPLEX); cudaMemset(x_out, 0, BUF_NTOTAL_COMPLEX);
    dcomplex *y0; cudaMalloc(&y0, BUF_NTOTAL_COMPLEX); cudaMemset(y0, 0, BUF_NTOTAL_COMPLEX);

    int grid = (int)((NTOTAL + TPB - 1) / TPB);

    printf("[LOG] ft_cffts2_kernel_3: NX=%d NY=%d NZ=%d ITERATIONS=%d\n", NX, NY, NZ, ITERATIONS);
    ft_kernel<<<grid, TPB, 0>>>(x_out, y0);
