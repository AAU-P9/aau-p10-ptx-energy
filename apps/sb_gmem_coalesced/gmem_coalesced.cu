...
// Minimal compute (just accumulate), power dominated by memory subsystem.
// 4MB array to stress DRAM bandwidth past L2 working set.
__global__ void KERNEL_LAUNCH_BOUNDS(256, 4)
bench_gmem_coalesced(const float * __restrict__ data, float *out, int dataLen) {
    KERNEL_META(bench_gmem_coalesced,
        ...
        _META_FRAG_CUSTOM("access_stride_bytes", "4")
    );
...
int main() {
    ...
    bench_gmem_coalesced<<<grid_size, block_size>>>(d_data, d_out, dataLen);
