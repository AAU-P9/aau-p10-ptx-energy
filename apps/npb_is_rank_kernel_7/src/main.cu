...
__global__ void is_kernel(INT_TYPE* partial_verify_vals,
...
int main() {
    ...
    INT_TYPE *rank_array; cudaMalloc(&rank_array, BUF_TEST_ARRAY); cudaMemset(rank_array, 0, BUF_TEST_ARRAY);
    INT_TYPE *passed_verification; cudaMalloc(&passed_verification, sizeof(INT_TYPE)); cudaMemset(passed_verification, 0, sizeof(INT_TYPE));
    ...
    cudaFree(partial_verify_vals); cudaFree(key_buff1); cudaFree(rank_array); cudaFree(passed_verification);
    return 0;
}
