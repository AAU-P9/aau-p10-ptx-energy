...
matrix(int *A){
  ...
  META_ASSUME("only thread (0,0) per block writes");

	/* To access only to one thread per block*/
	if (threadIdx.x==0 && threadIdx.y==0){

		/* Stotring the final result*/
		A[(blockIdx.y*blockDim.y + threadIdx.y)*_M+(blockIdx.x*blockDim.x + threadIdx.x)]= KERNEL_NUMBER;
