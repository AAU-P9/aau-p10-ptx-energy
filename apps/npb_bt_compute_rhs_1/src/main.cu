...
__global__ void bt_kernel(double* rho_i_device,
		double* us_device, 
		double* vs_device,
		double* ws_device, 
		double* qs_device,
...
int main() {
    ...
    double *rho_i, *us, *vs, *ws, *qs, *square, *u;
    ...
    size_t tpb = 32;
    size_t wx = round_work(IMAX, tpb);
    size_t wy = round_work(JMAX, 1);
    size_t wz = round_work(KMAX, 1);
    dim3 block(wx/tpb, wy, wz);
    dim3 thread(tpb, 1, 1);

    printf("[LOG] bt_compute_rhs_1: PROBLEM_SIZE=%d, ITERATIONS=%d\n", PROBLEM_SIZE, ITERATIONS);
    bt_kernel<<<block, thread>>>(rho_i, us, vs, ws, qs, square, u);
    ...
    cudaFree(rho_i); cudaFree(us); cudaFree(vs); cudaFree(ws);
