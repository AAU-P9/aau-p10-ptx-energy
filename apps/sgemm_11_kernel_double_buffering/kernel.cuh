...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  META_BEGIN_KERNEL(sgemmDoubleBuffering);
  META_PARAM_INT(0, M, TYPE_I32, rows_A, RANGE(1, 65536) MULTIPLE(128));
  META_PARAM_INT(1, N, TYPE_I32, cols_B, RANGE(1, 65536) MULTIPLE(256));
  ...
  META_SHARED(As, ELEM_F32, 2 *BM *BK * 4);
  META_SHARED(Bs, ELEM_F32, 2 *BK *BN * 4);
  META_LAYOUT(A, LAYOUT_ROW_MAJOR, "M x K");
  META_LAYOUT(B, LAYOUT_ROW_MAJOR, "K x N");
  META_LAYOUT(C, LAYOUT_ROW_MAJOR, "M x N");
  META_ASSUME("NUM_THREADS == 256 && BM == 128 && BN == 256 && BK == 16");
  META_ASSUME("double-buffered SMEM: threads split into two groups for overlap");
  ...
  META_END_KERNEL(sgemmDoubleBuffering);
}