...
__global__ void ft_kernel(const int is, 
		dcomplex gty1[], 
		dcomplex gty2[], 
		dcomplex u_device[]){
	...
	META_LOOP(l_sweep_1, 1, 30, false);
	for(l=1; l<=logd2; l+=2){
		n1 = NY / 2;
		lk = 1 << (l - 1);
		li = 1 << (logd2 - l);
		lj = 2 * lk;
		ku = li;
		META_LOOP(i1_loop, 1, NX, false);
		for(i1=0; i1<=li-1; i1++){
			META_LOOP(k1_loop, 1, NX, false);
			for(k1=0; k1<=lk-1; k1++){
				i11 = i1 * lk;
				i12 = i11 + n1;
				i21 = i1 * lj;
		...
		if(l==logd2){
			...
			lj = 2 * lk;
			ku = li;
			META_LOOP(i1_loop_1, 1, NX, false);
			for(i1=0; i1<=li-1; i1++){
				META_LOOP(k1_loop_1, 1, NX, false);
				for(k1=0; k1<=lk-1; k1++){
					i11 = i1 * lk;
					i12 = i11 + n1;
					i21 = i1 * lj;
...
int main() {
    ...
    dcomplex *y0; cudaMalloc(&y0, BUF_NTOTAL_COMPLEX); cudaMemset(y0, 0, BUF_NTOTAL_COMPLEX);
    dcomplex *y1; cudaMalloc(&y1, BUF_NTOTAL_COMPLEX); cudaMemset(y1, 0, BUF_NTOTAL_COMPLEX);
    dcomplex *u; cudaMalloc(&u, BUF_U_COMPLEX); cudaMemset(u, 0, BUF_U_COMPLEX);

    int grid = (int)(((size_t)NX*NZ + TPB - 1) / TPB);

    printf("[LOG] ft_cffts2_kernel_2: NX=%d NY=%d NZ=%d ITERATIONS=%d\n", NX, NY, NZ, ITERATIONS);
    ft_kernel<<<grid, TPB, 0>>>(1, y0, y1, u);
    ...
    cudaFree(y0); cudaFree(y1); cudaFree(u);
    return 0;
}
