...
__global__ void is_kernel(INT_TYPE* key_buff2,
...
int main() {
    ...
    INT_TYPE *key_buff2; cudaMalloc(&key_buff2, BUF_KEY_BUFF2); cudaMemset(key_buff2, 0, BUF_KEY_BUFF2);
    ...
    printf("[LOG] is_full_verify_kernel_2: CLASS=%c TOTAL_KEYS=%d MAX_KEY=%d ITERATIONS=%d\n",
           CLASS, TOTAL_KEYS, MAX_KEY, ITERATIONS);
    is_kernel<<<grid, tpb, 0>>>(key_buff2, key_buff1, key_array, grid, NUM_KEYS);
    ...
    cudaFree(key_buff2); cudaFree(key_buff1); cudaFree(key_array);
    return 0;
}
