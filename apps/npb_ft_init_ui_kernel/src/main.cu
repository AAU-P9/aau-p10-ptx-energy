...
int main() {
    ...
    dcomplex *u1; cudaMalloc(&u1, BUF_NTOTAL_COMPLEX); cudaMemset(u1, 0, BUF_NTOTAL_COMPLEX);
    double *twiddle; cudaMalloc(&twiddle, BUF_NTOTAL_DOUBLE); cudaMemset(twiddle, 0, BUF_NTOTAL_DOUBLE);

    int grid = (int)((NTOTAL + TPB - 1) / TPB);

    printf("[LOG] ft_init_ui_kernel: NX=%d NY=%d NZ=%d ITERATIONS=%d\n", NX, NY, NZ, ITERATIONS);
    for (int it = 0; it < ITERATIONS; it++) {
        ft_kernel<<<grid, TPB, 0>>>(u0, u1, twiddle);
    }
    ...
    cudaFree(u0); cudaFree(u1); cudaFree(twiddle);
    return 0;
}
