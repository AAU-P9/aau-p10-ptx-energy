...
__global__ void bt_kernel(double r[],
		double* rho, 
		double global_data[]){
...
int main() {
    ...
    double *r, *rho, *global_data;
    cudaMalloc(&r,           BUF_VEC);   cudaMemset(r,           0, BUF_VEC);
    cudaMalloc(&rho,         sizeof(double)); cudaMemset(rho,    0, sizeof(double));
    cudaMalloc(&global_data, BUF_GDATA); cudaMemset(global_data, 0, BUF_GDATA);

    int grid = (NA + TPB - 1) / TPB;
    int thread = TPB;

    printf("[LOG] cg_kernel_two: NA=%d, ITERATIONS=%d\n", NA, ITERATIONS);
    bt_kernel<<<grid, thread, TPB*sizeof(double)>>>(r, rho, global_data);
    ...
    cudaFree(r); cudaFree(rho); cudaFree(global_data);
    return 0;
}
